...
function DialogRoot({ state, close }: { state: DialogState; close: () => void }) {
	...
	return createPortal(
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			{/* overlay */}
			<button aria-label="Fechar" onClick={close} className={dialogOverlay} aria-hidden="true" tabIndex={-1} />
			{/* content */}
			<div
				role="dialog"
				aria-modal="true"
			...
			>
				...
				<div id="dialog-description" className="px-5 py-4 text-sm text-zinc-700">
					{typeof state.description === "string" ? <p>{state.description}</p> : state.description}
				</div>
			</div>
		</div>,
		document.body
