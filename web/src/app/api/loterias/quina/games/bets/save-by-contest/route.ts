...
export async function POST(request: Request) {
  ...
  // Replace items
  const { error: delErr } = await supabase.from('quina_bet_list_items').delete().eq('list_id', listId);
  ...
  for (let i = 0; i < payload.length; i += 1000) {
    ...
    const { error: insErr } = await supabase.from('quina_bet_list_items').insert(batch);
