...
export async function POST(request: Request) {
  ...
  // Verify ownership and get existing items count
  const { data: setData, error: setErr } = await supabase
    .from('lotofacil_user_sets')
    .select('id, user_id, source_numbers')
    .eq('id', setId)
    .single();
  if (setErr || !setData)
  ...
  const maxPosition =
    existingItems && existingItems.length > 0
      ? (existingItems[0].position as number)
      : -1;
  ...
  // Offset positions to append after existing
  const items = chosen.map((pos, idx) => {
    const idxs = allIdxComb[pos];
    const nums = idxs.map((ii) => uniq[ii]).sort((a, b) => a - b);
    return { position: maxPosition + 1 + idx, numbers: nums };
  });
  ...
  const { data: countData } = await supabase
    ...
    .eq('set_id', setId);
  const newSampleSize = countData?.length ?? 0;
  await supabase
    .from('lotofacil_user_sets')
    .update({ sample_size: newSampleSize })
    .eq('id', setId);
