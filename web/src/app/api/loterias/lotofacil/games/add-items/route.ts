...
export async function POST(request: Request) {
  ...
  // Build normalized bets exactly as provided (length 15..20), no combination expansion
  ...
  for (const arr of items) {
    if (!Array.isArray(arr) || arr.length < 15 || arr.length > 20) continue;
  ...
  if (betsFromPayload.length === 0) {
    return NextResponse.json(
      { error: 'Nenhuma aposta válida (15..20 dezenas únicas de 1 a 25).' },
      { status: 400 },
    );
  }
