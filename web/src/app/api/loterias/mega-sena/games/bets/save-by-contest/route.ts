...
export async function POST(request: Request) {
  ...
  let listId: string | null = null;
  {
    ...
    if (existing) {
      ...
      const { data: created, error: insErr } = await supabase
        .from('megasena_bet_lists')
        .insert({ user_id: user.id, contest_no: contestNo, title: title ?? null })
        .select('id')
        .single();
      if (insErr || !created)
        return NextResponse.json({ error: insErr?.message || 'Cannot create list' }, { status: 500 });
  ...
  await supabase.from('megasena_bet_list_items').delete().eq('list_id', listId!);
  if (rows.length > 0) {
    const { error: insErr } = await supabase.from('megasena_bet_list_items').insert(rows);
