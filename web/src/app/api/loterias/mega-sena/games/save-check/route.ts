...
export async function POST(request: Request) {
  ...
  if (existListErr)
    return NextResponse.json({ error: existListErr.message }, { status: 500 });
  ...
  if (Array.isArray(existingList) && existingList.length > 0) {
    ...
    if (sameRow) {
      ...
      return NextResponse.json(
        {
          error:
            'A saved check for this contest exists with a different draw. Clear it before saving another draw.',
        },
        { status: 409 },
      );
    }
  ...
  if (!checkId) {
    ...
    if (chkErr || !checkIns)
      return NextResponse.json(
        { error: chkErr?.message || 'Cannot create check' },
        { status: 500 },
      );
