...
export async function POST(request: Request) {
  ...
  // Cap at 5000 combinations for performance
  ...
  const { data: setInsert, error: setErr } = await supabase
    ...
    .insert({
      user_id: user.id,
      source_numbers: uniq, // Preserve order of insertion
