...
export function StudiesSidebar({
  ...
  const descriptions = useMemo<Record<string, string>>(
    () => ({
      overdue_dezena:
        'Lista as dezenas com maior tempo desde a última ocorrência (1-100).',
      pair_freq:
      ...
      sum_range_50:
        'Faixas de soma (largura 50) mais comuns entre as 20 dezenas sorteadas.',
      parity_comp:
        'Composições de pares/ímpares mais frequentes (ex.: 10p-10i).',
      window200_hot: 'Dezenas mais frequentes nos últimos 200 concursos.',
      decade_dist:
        'Distribuição de dezenas por décadas (01–10, 11–20, ..., 91–100).',
      last_digit:
  ...
  async function loadStudy(key: string) {
    ...
    try {
      const res = await fetch(
        `/api/loterias/lotomania/studies?study_key=${encodeURIComponent(
          key,
        )}&limit=60`,
      );
  ...
  async function exportAllStudies() {
    setExporting(true);
    try {
      const totalRes = await fetch('/api/loterias/lotomania/total-draws');
      ...
      link.href = url;
      link.download = `estudos_lotomania_${new Date().toISOString().split('T')[0]}.csv`;
      ...
      <div className='mb-3 flex items-center justify-between gap-2'>
        ...
        <div className='flex items-center gap-2 min-w-0'>
          ...
          <button
            ...
            className='rounded-md border border-white/20 bg-white/10 px-3 py-1.5 text-xs text-zinc-200 hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors group relative'
            aria-label='Exportar todos os estudos'
          >
            {exporting ? 'Exportando...' : 'Exportar'}
            {!exporting && (
              <span className='absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block z-50 w-64 p-2 text-xs text-zinc-900 bg-white rounded-md shadow-lg border border-black/10 pointer-events-none whitespace-normal'>
                Exportar todos os estudos em um único arquivo CSV.
              </span>
            )}
          </button>
        </div>
