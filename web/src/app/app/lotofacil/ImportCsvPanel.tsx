...
export function ImportCsvPanel() {
  ...
  async function runImport() {
    ...
    setBusyMsg('Processando importação da Lotofácil…');
    setBusy(true);
    try {
      const text = await file.text();
      const res = await fetch('/api/loterias/lotofacil/import', {
        method: 'POST',
  ...
  async function clearAll() {
    if (confirmClear === 0) {
      setConfirmClear(1);
      setStatus(
        'Confirma limpar toda a base da Lotofácil? Clique novamente para confirmar.',
      );
      return;
    }
    if (confirmClear === 1) {
    ...
    setBusyMsg('Limpando base da Lotofácil…');
    setBusy(true);
    try {
      const res = await fetch('/api/loterias/lotofacil/clear', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  return (
    <section className='rounded-lg border border-[var(--black-30)] bg-[var(--black-20)] p-4 space-y-3'>
      ...
      <div className='text-sm text-zinc-200'>
        Importação da base de sorteios — Lotofácil
      </div>
      <p className='text-xs text-zinc-400'>
        Envie o CSV da Lotofácil com o cabeçalho fornecido. A validação completa e a
