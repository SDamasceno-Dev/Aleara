...
export function StudiesSidebar({
  ...
  const descriptions = useMemo<Record<string, string>>(
    () => ({
      ...
      sum_range_20:
        'Faixas de soma (largura 20) mais comuns entre as 15 dezenas.',
      parity_comp: 'Composições de pares/ímpares mais frequentes (ex.: 8p-7i).',
      window200_hot: 'Dezenas mais frequentes nos últimos 200 concursos.',
      decade_dist: 'Distribuição de dezenas por faixas (01–10, 11–20, 21–25).',
      last_digit:
  ...
  async function loadStudy(key: string) {
    ...
    try {
      const res = await fetch(
        `/api/loterias/lotofacil/studies?key=${encodeURIComponent(key)}`,
      );
      const data = await res.json();
      setStudyTitle(data?.study?.title ?? key);
      setItems(
        ((data?.study?.items ?? []) as Array<{
          rank: number;
          item_key: string;
          value: number;
        }>) ?? [],
  ...
  async function exportAllStudies() {
    ...
    try {
      ...
      const totalRes = await fetch('/api/loterias/lotofacil/total-draws');
      const totalData = await totalRes.json();
      const totalJogos = (totalData?.total as number | undefined) ?? 0;
      ...
      for (const study of allStudies) {
        ...
        const res = await fetch(
          `/api/loterias/lotofacil/studies?key=${encodeURIComponent(studyKey)}`,
        );
        const data = await res.json();
      ...
      link.href = url;
      link.download = `estudos_lotofacil_${new Date().toISOString().split('T')[0]}.csv`;
