'use client';

import { useRef, useState } from 'react';
import { Button } from '@/components/button';
import { useRouter } from 'next/navigation';

export function ImportCsvPanel() {
  const router = useRouter();
  const inputRef = useRef<HTMLInputElement | null>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [status, setStatus] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [confirmClear, setConfirmClear] = useState<0 | 1 | 2>(0);

  async function onFileSelected(file: File) {
    setFileName(file.name);
    setStatus(null);
  }

  async function runImport() {
    const file = inputRef.current?.files?.[0];
    if (!file) {
      setStatus('Selecione um arquivo CSV primeiro.');
      return;
    }
    setBusy(true);
    try {
      const text = await file.text();
      const res = await fetch('/api/loterias/mega-sena/import', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ csv: text }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.status === 501) {
        setStatus(
          data?.message ??
            'Endpoint de importação ainda não implementado. Em breve processaremos o CSV.',
        );
      } else if (!res.ok) {
        setStatus(data?.error ?? 'Falha ao importar.');
      } else {
        setStatus(
          `Importação concluída: inseridos ${data?.imported ?? 0}, atualizados ${data?.updated ?? 0}, ignorados ${data?.skipped ?? 0}.`,
        );
        // Refresh to update server-rendered DataPanel when user switches tabs
        router.refresh();
      }
    } catch (e) {
      setStatus('Erro ao ler/enviar o arquivo.');
    } finally {
      setBusy(false);
    }
  }

  async function clearAll() {
    if (confirmClear === 0) {
      setConfirmClear(1);
      setStatus('Confirma limpar toda a base desta loteria? Clique novamente para confirmar.');
      return;
    }
    if (confirmClear === 1) {
      setConfirmClear(2);
      setStatus('Última confirmação: esta ação é permanente. Clique mais uma vez para prosseguir.');
      return;
    }
    setBusy(true);
    try {
      const res = await fetch('/api/loterias/mega-sena/clear', { method: 'DELETE' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setStatus(data?.error ?? 'Falha ao limpar a base.');
      } else {
        setStatus('Base limpa com sucesso.');
        // reset file selection to avoid accidental re-import
        if (inputRef.current) inputRef.current.value = '';
        setFileName(null);
        router.refresh();
      }
    } catch {
      setStatus('Erro ao limpar a base.');
    } finally {
      setBusy(false);
      setConfirmClear(0);
    }
  }

  return (
    <section className='rounded-lg border border-[var(--black-30)] bg-[rgb(from var(--black) r g b / 0.15)] p-4 space-y-3'>
      <div className='text-sm text-zinc-200'>Importação da base de sorteios</div>
      <p className='text-xs text-zinc-400'>
        Envie o CSV da Mega-Sena com o cabeçalho fornecido. A validação completa e a
        importação em lotes serão realizadas no servidor.
      </p>
      <div className='flex items-center gap-2 flex-wrap'>
        <input
          ref={inputRef}
          type='file'
          accept='.csv,text/csv'
          onChange={(e) => {
            const f = e.target.files?.[0];
            if (f) onFileSelected(f);
          }}
          className='text-sm border border-[var(--black-30)] p-1 rounded-md cursor-pointer'
        />
        <Button type='button' intent='primary' size='sm' disabled={busy} onClick={runImport}>
          {busy ? 'Enviando…' : 'Iniciar importação'}
        </Button>
        <Button
          type='button'
          intent='danger'
          variant='outline'
          size='sm'
          disabled={busy}
          onClick={clearAll}
        >
          {confirmClear === 0 ? 'Limpar base' : confirmClear === 1 ? 'Confirmar limpeza' : 'Confirmar (final)'}
        </Button>
      </div>
      <div className='text-xs text-zinc-500'>
        {fileName ? `Arquivo selecionado: ${fileName}` : 'Nenhum arquivo selecionado'}
      </div>
      <div role='status' aria-live='polite' className='text-xs text-zinc-300'>
        {status}
      </div>
    </section>
  );
}


