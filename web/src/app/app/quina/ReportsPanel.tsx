...
type ReportData = {
  ...
  draw: number[];
  checkedAt: string;
  kpis: { total: number; c2: number; c3: number; c4: number; c5: number; hitRate: number };
  items: ReportItem[];
};

type AggregateRow = { contestNo: number; checkedAt: string; total: number; c2: number; c3: number; c4: number; c5: number; hitRate: number };
type AggregateData = {
  kpis: { totalConferences: number; totalBets: number; avgPerCheck: number; c2: number; c3: number; c4: number; c5: number; hitRate: number };
  rows: AggregateRow[];
};
...
export default function ReportsPanel() {
  ...
  async function loadLatest() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/loterias/quina/reports/latest', { cache: 'no-store' });
  ...
  async function loadByContest(n: number) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/loterias/quina/reports/by-contest?${new URLSearchParams({ contestNo: String(n) })}`, {
        cache: 'no-store',
      });
      const data = await fetchToJson(res);
      if (!res.ok) {
  ...
  async function loadAggregate() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/loterias/quina/reports/aggregate', { cache: 'no-store' });
      const data = await fetchToJson(res);
      if (!res.ok) {
      ...
      if (data?.empty) {
        setAggregate({ kpis: { totalConferences: 0, totalBets: 0, avgPerCheck: 0, c2: 0, c3: 0, c4: 0, c5: 0, hitRate: 0 }, rows: [] });
      } else {
  ...
  const drawStr = useMemo(() => (report?.draw ?? []).map((n) => String(n).padStart(2, '0')).join(', '), [report]);

  return (
    <section className='rounded-lg border border-border/60 bg-card/90 p-4'>
      <LoadingOverlay show={busy} message={viewMode === 'aggregate' ? 'Carregando…' : 'Gerando…'} />
      ...
      <div className='mb-3 flex flex-wrap items-end gap-3'>
        ...
        <div className='flex gap-2'>
          <button className='rounded-md border border-white-10 px-3 py-1 text-sm hover:bg-white-10' disabled={!contestInput || loading} onClick={() => loadByContest(Number(contestInput))}>
            Carregar por concurso
          </button>
          <button className='rounded-md border border-white-10 px-3 py-1 text-sm hover:bg-white-10' disabled={loading} onClick={loadLatest}>
            Última conferência
          </button>
          <button className='rounded-md border border-white-10 px-3 py-1 text-sm hover:bg-white-10' disabled={loading} onClick={loadAggregate}>
            Relatório geral
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        !aggregate || aggregate.kpis.totalConferences === 0 ? (
          ...
          <>
            <div className='mb-3 grid grid-cols-2 gap-2 md:grid-cols-6'>
              <KpiCard label='Conferências' value={aggregate.kpis.totalConferences} />
              <KpiCard label='Apostas' value={aggregate.kpis.totalBets} />
              <KpiCard label='Média/conferência' value={aggregate.kpis.avgPerCheck.toFixed(1)} />
              <KpiCard label='Acertos 2' value={aggregate.kpis.c2} />
              <KpiCard label='Acertos 3' value={aggregate.kpis.c3} />
              <KpiCard label='Acertos 4+' value={aggregate.kpis.c4 + aggregate.kpis.c5} />
            </div>
            <PieSummary c2={aggregate.kpis.c2} c3={aggregate.kpis.c3} c4={aggregate.kpis.c4} c5={aggregate.kpis.c5} total={aggregate.kpis.totalBets} />
        ...
        <>
          ...
          <div className='mb-2 text-xs text-zinc-400'>
            Sorteio: <span className='text-zinc-200'>{drawStr || '—'}</span> • <span>Conferido em {new Date(report.checkedAt).toLocaleString()}</span>
          </div>
          <div className='rounded-md border border-white/10'>
            <div className='overflow-x-auto'>
              <table className='min-w-full text-sm'>
...
function KpiCard({ label, value, highlight }: { label: string; value: string | number; highlight?: boolean }) {
  return (
    <div className='rounded-md border border-white/10 p-3'>
      ...
      <div className={`text-lg font-semibold ${highlight ? 'text-green-300' : 'text-zinc-100'}`}>{value}</div>
    </div>
  );
}
...
function PieSummary({ c2, c3, c4, c5, total }: { c2: number; c3: number; c4: number; c5: number; total: number }) {
  const sum = c2 + c3 + c4 + c5;
  if (sum === 0 || total === 0) {
    return <div className='mb-3 pot-12 text-sm text-zinc-400'>Sem dados suficientes para gráfico de pizza.</div>;
  }
  const size = 180;
  const stroke = 20;
  ...
  return (
    <div className='mb-4 grid grid-cols-1 gap-4 md:grid-cols-2'>
      <div className='flex items-center justify-center'>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className='-o rotate-90'>
          <circle cx={cx} cy={cy} r={r} fill='none' stroke='rgba(255,255,255,0.08)' strokeWidth={stroke} />
          <circle cx={cx} cy={cy} r={r} fill='none' stroke='#a3e635' strokeWidth={stroke} strokeDasharray={`${s2} ${circumference - s2}`} strokeDashoffset={off2} />
          <circle cx={cx} cy={cy} r={r} fill='none' stroke='#f59e0b' strokeWidth={stroke} strokeDasharray={`${s3} ${circumference - s3}`} strokeDashoffset={off3} />
          <circle cx={cx} cy={cy} r={r} fill='none' stroke='#f97316' strokeWidth={stroke} strokeDasharray={`${s4} ${circumference - s4}`} strokeDashoffset={off4} />
          <circle cx={cx} cy={cy} r={r} fill='none' stroke='#22c55e' strokeWidth={stroke} strokeDasharray={`${s5} ${circumference - s5}`} strokeDashoffset={off5} />
      ...
      <div className='flex flex-col justify-center gap-2 text-sm'>
        ...
        <div className='mt-2 text-xs text-zinc-400'>Base: {total} apostas conferidas</div>
      </div>
    </div>
  );
}

function Legend({ color, label, value, pct }: { color: string; label: string; value: any; pct?: any }) {
